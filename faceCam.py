...
def call_arduino(messageSerial):
    try :
        ARDUINO_SERIAL.write(messageSerial.encode())
        print ("Komunikasi serial arduino")
...
def vision_processing(frame):
    ...
    if process_this_frame:
        ...
        # Mode "cnn" for using GPU please compile DLIB using GPU, DLIB default using CPU
        ...
        if(len(face_encodings) < 1):
            threadCloseDoor = threading.Thread(target=call_arduino, args=("0",))
            threadCloseDoor.start()
            threadCloseDoor.join()
            print("Wajah tidak ditemukan dan kunci pintu")
            ...
            if True in matches:
                ...
                for i, face_distance in enumerate(face_distances):
                    ...
                    if(face_distance < 0.5):
                        ...
                        if(cf < 1):
                            pass # do something if no data in SQLite database

                        # if check database not found
                        if(face_id > 0):
                            print("Wajah ditemukan dan buka kunci pintu")
                            threads.append(threading.Thread(target=call_arduino, args=("1",)))
                            threads.append(threading.Thread(target=save_face, args=(face_id, name,)))
                        else :
                            print("Wajah tidak ditemukan dan kunci pintu")
                            threads.append(threading.Thread(target=call_arduino, args=("0",)))
